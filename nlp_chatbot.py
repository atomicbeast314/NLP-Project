...
Tasks covered:
  ...
  3. Sample conversations included at the end of execution
...
Approach:
  ...
  - Flexible pattern matching allows the bot to handle unseen questions
    (not limited to hard-coded inputs)
"""
...
# ── Knowledge base ──────────────────────────────────────────────────────────
# Each intent has:
#   - patterns : example user inputs (more = better generalization)
#   - responses: possible bot replies (one chosen at random)
...
# Flatten all patterns and map each to its intent
all_patterns = []
pattern_intents = []
...
def classify_intent(user_input):
    ...
    return pattern_intents[best_idx], best_score
...
SAMPLE_QUERIES = [
    ...
    "xyz abc nonsense input",
]


def run_sample_conversations():
    """Run and display sample conversations to demonstrate the chatbot."""
    print("\n" + "=" * 70)
    print("  SAMPLE CONVERSATIONS")
    print("=" * 70)

    for query in SAMPLE_QUERIES:
        response, intent, score = get_response(query)
        print(f"\n  You   : {query}")
        print(f"  Bot   : {response}")
        print(f"  [Intent: {intent or 'unknown'} | Confidence: {score:.3f}]")
...
def interactive_chat():
    """Run the chatbot in interactive mode, accepting live user input."""
    print("\n" + "=" * 70)
    print("  NLP CHATBOT — Interactive Mode")
    print("  Ask me about NLP concepts or AI applications!")
    ...
    while True:
        ...
        response, intent, score = get_response(user_input)
        print(f"Bot: {response}")
        print(f"     [Intent: {intent or 'unknown'} | Confidence: {score:.3f}]\n")
...
if __name__ == "__main__":
    print("=" * 70)
    print("  CSE3246 — NLP Assignment: Chatbot using NLP")
    print("  Intent-based chatbot with TF-IDF + Cosine Similarity")
    print("=" * 70)

    # Show sample conversations first
    run_sample_conversations()
